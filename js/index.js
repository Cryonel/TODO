...
function rewrite() {
    ...
    $.each(data,function (index,val) {
        if(state === "wait") {
            if(val.isDone === 0){
                str += "<li  id="+index+"><div class='time'><span>"+getdate(val.time)+"</span><span>"+getTime(val.time)+"</span></div><span class='sp3'>"+val.con+ "</span><div class='right'>完成</div></li>";
            }
        }else if(state ==="down"){
            if(val.isDone ===1){
                str += "<li style='background:#ccc' id="+index+"><div class='time'><span>"+getdate(val.time)+"</span><span>"+getTime(val.time)+"<br></span></div><span class='sp3'>"+val.con+ "</span><div class='del'>删除</div></li>";
            }
        }
    });
    $(".content ul").html(str);
